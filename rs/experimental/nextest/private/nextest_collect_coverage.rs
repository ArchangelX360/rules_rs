...
//!
//! Bazel invokes this through `CC_CODE_COVERAGE_SCRIPT`. It takes the instrumented objects from
//! the environment, since `TEST_BINARY` is the runner symlink and carries no instrumented code,
//! and one `nextest_test` may drive several test binaries.
//!
//! Behaviour otherwise follows rules_rust's `//util/collect_coverage`, so `bazel coverage`
//! reports the same as it does for `rust_test`.
//!
//! Environment:
...
fn main() {
    ...
    // --instrumentation_filter, produces no .profraw files. `llvm-profdata merge` fails on empty
    // input, so write an empty report.
