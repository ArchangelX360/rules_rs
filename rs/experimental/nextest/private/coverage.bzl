...
Follows `rust_test` (`rules_rust`'s `rust/private/rust.bzl`), with the instrumented objects
passed explicitly: under nextest `TEST_BINARY` is this rule's runner, which carries no
instrumented code, and one target may drive several test binaries.
"""
...
def _toolchain_tool_path(file):
    ...
    Toolchain tools live in external repositories, so stripping `../` from the short path yields
    a runfiles-root-relative path.
    """
...
def coverage_env(ctx, toolchain, objects):
    ...
    return {
        ...
        # The collector resolves these against the runfiles root, so they carry the repository
        # component that a short path omits.
